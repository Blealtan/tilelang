...
//     Ok(())
// })?;
// __ctx.for_each(T::parallel([2, 3]), |__ctx, __vars| { let (i, j) = FromLoopVars::bind2(__vars)?; ...; Ok(()) })?;
// __ctx.for_each(T::pipelined(0, 2), |__ctx, __vars| { let i = FromLoopVars::bind1(__vars)?; ...; Ok(()) })?;
// __ctx.finish_ir_module()
